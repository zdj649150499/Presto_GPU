...
void *producer_thread(void *arg) 
{
    ...
    fcomplex *current_obs_fft_gpu;
    fcomplex *current_obs_fft_gpu_tmp;
    ...
    for (kk = 0; kk < fornum; kk++) {
        ...
        // 初始化并处理文件，生成候选体
        create_accelobs_list_2(&current_obs, &current_idata, cmd, 1, kk);
        ...
        {
            rstep = current_obs.corr_uselen * ACCEL_DR;
            ...
            printf("Searching with up to %d harmonics summed:\n",
                    1 << (current_obs.numharmstages - 1));
            printf("  f = %.1f to %.1f Hz\n", current_obs.rlo / current_obs.T, current_obs.rhi / current_obs.T);
            printf("  r = %.1f to %.1f Fourier bins\n", current_obs.rlo, current_obs.rhi);
            printf("  z = %.1f to %.1f Fourier bins drifted\n", current_obs.zlo, current_obs.zhi);
            if (current_obs.numw)
                printf("  w = %.1f to %.1f Fourier-derivative bins drifted\n", current_obs.wlo, current_obs.whi);
            ...
            if(firsttime)
            {
                ...
                if(cmd->cudaP)
                {
                    ...
                    cudaMalloc((void**)&outpows_gpu, sizeof(float)*subharminfs[0][0].numkern*current_obs.fftlen);  // only for ffdot->numrs*ffdot->numzs is useful, numrs is changed each time but low than fftlen, numkern = numzs;
                    cudaMemset(outpows_gpu, 0.0, sizeof(float)*subharminfs[0][0].numkern*current_obs.fftlen);
                    ...
                    if(cmd->inmemP)
                    {
                        ...
                        cudaMalloc((void**)&rinds_gpu, sizeof(int)*fftlen*(pow(2,current_obs.numharmstages)-1));
                        cudaMalloc((void**)&outpows_gpu_obs, sizeof(float)*(current_obs.highestbin+current_obs.corr_uselen)*current_obs.numbetween*current_obs.numz);
                    }
                    else
                    {
                        cudaMalloc((void**)&d_rinds_gpu, sizeof(unsigned short)*current_obs.corr_uselen*(pow(2,current_obs.numharmstages)-1));
                        cudaMalloc((void**)&d_zinds_gpu, sizeof(unsigned short)*current_obs.numz*(pow(2,current_obs.numharmstages)-1));
                        cudaMallocHost((void**)&d_rinds_cpu, sizeof(unsigned short)*current_obs.corr_uselen*(pow(2,current_obs.numharmstages)-1));
                        cudaMallocHost((void**)&d_zinds_cpu, sizeof(unsigned short)*current_obs.numz*(pow(2,current_obs.numharmstages)-1));

                        offset_array = (int **)malloc(current_obs.numharmstages * sizeof(int *));
                        offset_array[0] = (int *)malloc( 1 * sizeof(int) );
                        int jj;
                        for(ii=1; ii<current_obs.numharmstages; ii++){	
                                jj = 1 << ii;		
                                offset_array[ii] = (int *)malloc( jj * sizeof(int));
                        }
                        fkern_gpu =  cp_kernel_array_to_gpu(subharminfs, current_obs.numharmstages, offset_array);
                    }

                    if(current_obs.dat_input&&current_obs.numbins<100000000)
                    {
                        cudaMalloc((void**)&current_obs_fft_gpu, sizeof(cufftComplex)*(current_obs.numbins+ACCEL_PADDING));
                        current_obs_fft_gpu_tmp = current_obs_fft_gpu;
                    }

                    init_cuFFT_plans(subharminfs, current_obs.numharmstages, current_obs.inmem);
            ...
            if (cmd->zaplistP && !current_obs.mmap_file && current_obs.fft) {
                int numbirds;
                double *bird_lobins, *bird_hibins, hibin;

                /* Read the Standard bird list */
                numbirds = get_birdies(cmd->zaplist, current_obs.T, cmd->baryv,
                                    &bird_lobins, &bird_hibins);

                /* Zap the birdies */
                printf("Zapping them using a barycentric velocity of %.5gc.\n\n",
                    cmd->baryv);
                hibin = current_obs.N / 2;
                for (ii = 0; ii < numbirds; ii++) {
                    ...
                    if (bird_hibins[ii] >= hibin)
                        ...
                        zapbirds(bird_lobins[ii], bird_hibins[ii], NULL, current_obs.fft);
            ...
            if(current_obs.dat_input&&current_obs.numbins<100000000)
            {
                current_obs.fft_gpu = current_obs_fft_gpu_tmp + ACCEL_PADDING/2;
                cudaMemcpy(current_obs.fft_gpu-ACCEL_PADDING/2, current_obs.fft-ACCEL_PADDING/2, sizeof(cufftComplex)*(current_obs.numbins+ACCEL_PADDING), cudaMemcpyHostToDevice);                
            }
...

            {
                    ...
                    while (startr < current_obs.rlo) {
                        ...
                        if(!cmd->cudaP)
                        {
                            fundamental = subharm_fderivs_vol(1, 1, startr, lastr,
                                                        &subharminfs[0][0], &current_obs);
                            fund_to_ffdot(fundamental, &current_obs);
                        ...
                        else 
                        {
                            ...
                            subharm_fderivs_vol_gpu(1, 1, startr, lastr, &subharminfs[0][0], &current_obs, fkern_gpu, pdata_gpu, tmpdat_gpu, tmpdat_gpu, outpows_gpu, outpows_gpu_obs, pdata, 0, d_zinds_gpu, d_rinds_gpu, d_zinds_cpu, d_rinds_cpu, fundamental, offset_array, 0, data_gpu, power);
                ...
                startr = current_obs.rlo;
                lastr = 0;
                nextr = 0;

            
                while (startr + rstep < current_obs.highestbin) {
                    ...
                    if(!cmd->cudaP)
                    {
                        fundamental = subharm_fderivs_vol(1, 1, startr, lastr,
                                                    &subharminfs[0][0], &current_obs);
                        cands = search_ffdotpows(fundamental, 1, &current_obs, cands);
                    }
                    else
                    {
                        fundamental = ini_subharm_fderivs_vol(1, 1, startr, lastr, &subharminfs[0][0], &current_obs);
                        subharm_fderivs_vol_gpu(1, 1, startr, lastr,
                                                    &subharminfs[0][0], &current_obs, fkern_gpu, pdata_gpu, tmpdat_gpu, tmpdat_gpu, outpows_gpu, outpows_gpu_obs, pdata, current_obs.inmem, d_zinds_gpu, d_rinds_gpu, d_zinds_cpu, d_rinds_cpu, fundamental, offset_array, 0, data_gpu, power);
                        
                        if(cmd->inmemP)
                        {
                            get_rinds_gpu(fundamental, rinds_gpu, current_obs.numharmstages);
                        }
                        else
                            get_rind_zind_gpu(d_rinds_gpu, d_zinds_gpu, d_rinds_cpu, d_zinds_cpu, current_obs.numharmstages, current_obs, startr);
                        nof_cand = search_ffdotpows_gpu(current_obs.powcut[twon_to_index(1)], outpows_gpu, cand_array_search_gpu, fundamental->numzs, fundamental->numrs, cand_gpu_cpu);
                        cands = search_ffdotpows_sort_gpu_result(fundamental, 1, &current_obs, cands, cand_gpu_cpu, nof_cand);
                    }
                    if (current_obs.numharmstages > 1) {        /* Search the subharmonics */

                        // Copy the fundamental's ffdot plane to the full in-core one
                        if (current_obs.inmem && !cmd->cudaP)
                            fund_to_ffdot(fundamental, &current_obs);

                        if(!cmd->cudaP)
                        {
                            for (stage = 1; stage < current_obs.numharmstages; stage++) {
                                harmtosum = 1 << stage;
                                for (harm = 1; harm < harmtosum; harm += 2) {
                                    if (current_obs.inmem) 
                                        inmem_add_subharm(fundamental, &current_obs, harmtosum, harm);
                                    else 
                                    {
                                ...
                                cands = search_ffdotpows(fundamental, harmtosum, &current_obs, cands);
                            }
                        }
                        else
                        {
                            for (stage = 1; stage < current_obs.numharmstages; stage++) {
                                harmtosum = 1 << stage;
                                if(cmd->inmemP)
                                    inmem_add_subharm_gpu(fundamental, &current_obs, outpows_gpu, outpows_gpu_obs, stage, rinds_gpu);
                                else
                                {
                                    for (harm = 1; harm < harmtosum; harm += 2)
                                        subharm_fderivs_vol_gpu(harmtosum, harm, startr, lastr,
                                                    &subharminfs[stage][harm-1], &current_obs, fkern_gpu, pdata_gpu, tmpdat_gpu, tmpdat_gpu, outpows_gpu, outpows_gpu_obs, pdata, 0, d_zinds_gpu, d_rinds_gpu, d_zinds_cpu, d_rinds_cpu, fundamental, offset_array, stage, data_gpu, power);
                                }
                                nof_cand = search_ffdotpows_gpu(current_obs.powcut[twon_to_index(harmtosum)], outpows_gpu, cand_array_search_gpu, fundamental->numzs, fundamental->numrs, cand_gpu_cpu);                        
                                cands = search_ffdotpows_sort_gpu_result(fundamental, harmtosum, &current_obs, cands, cand_gpu_cpu, nof_cand);
                            }
                        }
                    }
                    if(!cmd->cudaP)
                ...
                print_percent_complete(current_obs.highestbin - current_obs.rlo,
                                    current_obs.highestbin - current_obs.rlo, "search", 0);
        ...
        QueueItem item = { .cands = cands, .obs = current_obs, .idata = current_idata};
        ...
        if(kk == fornum-1)
        {
            ...
            {
                // if(current_obs.dat_input&&current_obs.numbins<100000000)
                //     cudaFree(current_obs_fft_gpu-ACCEL_PADDING/2);
                ...
                // if(current_obs.dat_input&&current_obs.numbins<100000000)
                //     cudaFree(current_obs.fft_gpu-ACCEL_PADDING/2);
                ...
                destroy_cuFFT_plans(subharminfs, current_obs.numharmstages, current_obs.inmem);
                // free(nof_cand);
            }
